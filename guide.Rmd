...
[ [follow up from intro2r](follow_up_from_intro.html) | [large data](large_data.html) | [git](/git/) ]

The outline, explanations, and code examples for this workshop are derived from the work found in the [_Data Visualization_ chapter](https://r4ds.had.co.nz/data-visualisation.html) and the [_Graphics for Communication_ chapter](https://r4ds.had.co.nz/graphics-for-communication.html) of Grolemund's and Wickham's [*R for Data Science*](https://r4ds.had.co.nz/).  Please reference Grolemund & Wickham as the source.  My goal with this document is to create an outline for a two hour workshop on `ggplot`.  Solutions to _R4DS_ exercises can be found in Jeffrey Arnold's [R for Data Science Solutions](https://jrnold.github.io/r4ds-exercise-solutions/).
...
Some [Follow-up](follow_up_from_intro.html) from last week
...
1. Make a scatter plot of `hwy` vs `cyl`. (Question from [R4DS](https://r4ds.had.co.nz/data-visualisation.html#exercises ) | [Solution](https://jrnold.github.io/r4ds-exercise-solutions/data-visualisation.html#exercise-3.2.4.))
...
```{r intro to levels and aethetics, echo=FALSE}
ggplot() + 
...
```{r shape by categorical, message=FALSE, warning=FALSE}
ggplot(data = mpg) + 
...
- Levels mapped inside the `aes()` are assigned dynamically from dataframe values.
...
([Question](https://r4ds.had.co.nz/data-visualisation.html#exercises-1)) What’s gone wrong with this code? Why are the points not blue? ([Solution](https://jrnold.github.io/r4ds-exercise-solutions/data-visualisation.html#exercise-3.3.1))
...
- `stroke` may not work with every [shape](https://r4ds.had.co.nz/data-visualisation.html#fig:shapes).  Check the help pages for more information
...
```{r layers, message=FALSE, warning=FALSE}
ggplot(data = mpg) + 
  geom_point(mapping = aes(x = displ, y = hwy, color = class)) + 
...
```{r layer on object, message=FALSE, warning=FALSE}
hwy_plot +
  geom_smooth(mapping = aes(x = displ, y = hwy))
```
...
```{r smooth via aethetic, message=FALSE, warning=FALSE}
hwy_plot +
...
Will these two graphs look different? Why/why not?  [Solution](https://r4ds.had.co.nz/data-visualisation.html#exercises-3)
...
Recreate the R code necessary to generate the graphs found at [3.6.1.6](https://r4ds.had.co.nz/data-visualisation.html#exercises-3) | [Solutions](https://jrnold.github.io/r4ds-exercise-solutions/data-visualisation.html#exercise-3.6.6)
...
```
{r a map, echo=TRUE, message=FALSE, warning=FALSE}
...
hwy_plot +
  geom_label(data = mpg %>% 
            ...
            aes(displ, hwy, label = model, color = class),
            nudge_x = -0.15,
            nudge_y = 1.50,
            show.legend = FALSE) +
  geom_label(data = mpg %>% 
...
- `xlim()` & `ylim()` are two simply ways to set the limits of the x and y scales
- another option is to use the `breaks()` and `labels()` functions as arguments inside one of the `scale_` functions.  e.g.

    - `scale_size_continuous(name="Engine Classification", 
                        breaks=c(1, 2, 3),
                        labels=c("1 Liter","2 Liter","3 Liter"))`
                      
The last two plots, below, use `xlim` to harmonize the x axis scale
...
```{r fun, message=FALSE, warning=FALSE}
ggplot(data = mpg, aes(x = displ, y = hwy)) + 
  geom_point(mapping = aes(color = class)) +
  geom_text(aes(label = label_text), 
            data = annotation, 
...
![Image Credit: Themes from _R for Data Science_ by Grolemund & Wickham](https://r4ds.had.co.nz/images/visualization-themes.png)
...
- Gallery of other extra cool [ggplot extensions](https://www.ggplot2-exts.org/gallery/)

## Resources

- [R for Data Science](https://r4ds.had.co.nz/) by Grolemund & Wickham.  Chapter on _Data Visualization_,  & Chapter on _Graphics for Communication_, & Chapter on _Exploratory Data Analysis_
- [ggplot2: part of the tidyverse](https://ggplot2.tidyverse.org/) -- check out all the cool on-board datasets
- [ggplot2: short tutorial](https://r-statistics.co/ggplot2-Tutorial-With-R.html) by Selva Prabhakaran
- [ggplot cookbook](https://www.cookbook-r.com/Graphs/)  by Winston Chang
- [Data Visualisation](https://rgup.gitlab.io/research_cycle/02_ggplot.html) (web resource)
- [**R Graph Gallery**](https://www.r-graph-gallery.com/)
- [Data Visualization](https://socviz.co/): A practical introduction by Kieran Healy (book)
...
The outline, explanations, and code examples for this workshop are from  **Grolemund's & Wickham's [*R for Data Science*](https://r4ds.had.co.nz/).  Please cite that source.**  This workbook outline serves as a guide for a two-hour workshop delivered at Duke University, [Data & Visualization Services](https://library.duke.edu/data/).  
